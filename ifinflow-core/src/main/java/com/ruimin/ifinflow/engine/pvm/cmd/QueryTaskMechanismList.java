...
public class QueryTaskMechanismList implements Command<List<TaskMechanismVO>> {
	...
	public List<TaskMechanismVO> execute(Environment environment)
			throws Exception {
		Session session = environment.get(Session.class);

		List<IWfUnit> units = (List<IWfUnit>) UserExtendsReference
				.getIdentityAdapter().getIdentityInfoPage(null, "3", 0, 0);
		...
		for (Object[] tmps : histasks) {
			if (tmps[0] == null) {
				tmvo = new TaskMechanismVO("未指定机构", "未指定机构", "0", "0", "0",
						"0", "0", "0");
			} else {
				tmvo = new TaskMechanismVO((String) tmps[0],
						unitMap.get(tmps[0]), "0", "0", "0", "0", "0", "0");
			}
			...
			if (tmps[1] != null) {
				...
				if ((1 == ti) || (2 == ti)) {
					tmvo.setTodoNum(StringHelper.stringPlusN(tmvo.getTodoNum(),
							(Integer) tmps[2]));
				} else if ((16 == ti) || (6 == ti)) {
					tmvo.setDoneNum(StringHelper.stringPlusN(tmvo.getDoneNum(),
							(Integer) tmps[2]));
				} else if (4 == ti) {
					tmvo.setBackNum(StringHelper.stringPlusN(tmvo.getBackNum(),
							(Integer) tmps[2]));
				} else if (512 == ti) {
					tmvo.setExceptionNum(StringHelper.stringPlusN(
							tmvo.getExceptionNum(), (Integer) tmps[2]));
				} else if (256 == ti) {
					tmvo.setOvertimeNum(StringHelper.stringPlusN(
							tmvo.getOvertimeNum(), (Integer) tmps[2]));
				}
			}

			tmvo.setTotal(StringHelper.stringPlusN(tmvo.getTotal(),
					(Integer) tmps[2]));
		}
