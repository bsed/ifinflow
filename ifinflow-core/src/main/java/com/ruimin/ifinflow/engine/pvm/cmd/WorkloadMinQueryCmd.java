...
public class WorkloadMinQueryCmd
    ...
    public String execute(Environment environment) throws Exception {
        ...
        Query query = session.createQuery("select wl from " + IFinFlowWorkloadEntity.class.getName() + "  wl where wl.staffId in (:staffIds) order by wl.workload asc");

        query.setParameterList("staffIds", this.staffs);
        List<IFinFlowWorkloadEntity> list = CollectionUtil.checkList(query.list(), IFinFlowWorkloadEntity.class);
        ...
        try {
            ...
            HistoryEvent.fire(new WorkloadUpdate(minWorkStaffId, minWorkStaffId, 1));
        } catch (Exception e) {
            throw new IFinFlowException(103039, e, new Object[]{Arrays.toString(this.staffs)});
        }
