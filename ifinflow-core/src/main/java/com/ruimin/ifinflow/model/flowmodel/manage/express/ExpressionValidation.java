 ...
 public class ExpressionValidation
 {
   ...
   public static boolean match(String toDeal)
     ...
     for (int i = 0; i < length; i++) {
       ...
       if (c == '(') {
         ...
         if (index == 0) {
           ...
           stack.add(Integer.valueOf(0));
           throw new IFinFlowException(110001, new Object[] { Integer.valueOf(i + 1) });
         }
       }
     }
     ...
     if (stack != null) {
       while (!stack.isEmpty()) {
         ...
         if (index != 0) {
           ...
           throw new IFinFlowException(110002, new Object[] { Integer.valueOf(index) });
         }
       }
     }
   ...
   {
     toDeal = toDeal.replaceAll("\\&\\&", "\\|\\|").replaceAll("\\(", "").replaceAll("\\)", "");
   ...
   public static boolean checkExpress(String toDeal, List<String> vars)
   {
     ...
     for (String atomExp : atomExps) {
       ...
       String leftExp = atomExp.substring(0, atomExp.indexOf(operator)).trim();
       ...
       String rightExp = atomExp.substring(atomExp.indexOf(operator) + operator.length());
       ...
       for (String splitStr : splits) {
         ...
         if (splitStr.startsWith("#{")) {
           if (!isVariable(splitStr, vars)) {
             throw new IFinFlowException(110004, new Object[] { splitStr });
           }
           
         }
         else if ((!isNumeric(splitStr)) && (
           (!splitStr.startsWith("\"")) || (!splitStr.endsWith("\"")))) {
           throw new IFinFlowException(110005, new Object[] { splitStr });
         }
       }
     }
