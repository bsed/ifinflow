 ...
 public class Validation
 {
   ...
   public boolean checkXml(String jpdlStr) throws IFinFlowException { log.trace("校验xml开始。。。 ");
     init(jpdlStr);
     ...
     for (Map.Entry<String, GraphVo> entry : this.graphMap.entrySet()) {
       ...
       if (null != graph) {
         ...
         String to; if ((null != transitions) && 
           (key.startsWith("ForkNode_"))) {
           if (transitions.size() < 2) {
             throw new IFinFlowException(110009, new Object[] { graph.getId() });
           }
           to = null;
           for (TransitionVo tvo : transitions) {
             if (null != tvo)
             {
               ...
               else if ((to.startsWith("ForkNode_")) && (to.equals(tvo.getTo()))) {
                 throw new IFinFlowException(110010, new Object[] { graph.getName(), graph.getId() });
               }
             }
           }
         }
   ...
   private void validSpecialNode(Set<String> forkOrDecisionSet, Set<String> joinOrSJoinSet, Set<String> endSet, String nodeType)
   {
     for (String nodeId : endSet) {
       this.endId = nodeId;
       if ("FORK".equals(nodeType)) {
         validForkJoinMatchs((GraphVo)this.graphMap.get(nodeId), forkOrDecisionSet, joinOrSJoinSet);
       } else {
         validDecisionSjoinMatchs((GraphVo)this.graphMap.get(nodeId), forkOrDecisionSet, joinOrSJoinSet);
       }
     }
   }
   ...
   private void validDecisionSjoinMatchs(GraphVo graph, Set<String> decisionSet, Set<String> sjoinSet)
   ...
   private void validDecisionSimpleJoinMatch(GraphVo graph, Set<String> decisionSet, Set<String> sjoinSet) throws IFinFlowException {
     ...
     for (int i = 0; i < priorNamesSize; i++)
     {
       ...
       if (priorName.startsWith("SJoinNode_")) {
         validDecisionSimpleJoinMatch((GraphVo)this.graphMap.get(priorName), decisionSet, sjoinSet);
       }
       else {
         ...
         if ((tempDecisionName != priorName) && (StringUtils.isNotBlank(tempDecisionName))) {
           throw new IFinFlowException(110013, new Object[0]);
         }
         if ((i == priorNamesSize - 1) && (tempDecisionName == priorName)) {
           if (((GraphVo)this.graphMap.get(tempDecisionName)).getAfterNameList().size() != ((GraphVo)this.graphMap.get(currentJoinName)).getPriorNameList().size()) {
             throw new IFinFlowException(110013, new Object[0]);
           }
           ...
           validDecisionSjoinMatchs((GraphVo)this.graphMap.get(this.endId), decisionSet, sjoinSet);
         }
       }
     }
   }
   
   private void validForkJoinMatchs(GraphVo graph, Set<String> forkSet, Set<String> joinSet) throws IFinFlowException {
   ...
   private void validJoin(String graphId, Set<String> forkSet, Set<String> joinSet) {
     ...
     for (String nodeId : tempPriorIdList) {
       ...
       if (!nodeId.startsWith("JoinNode_")) {
         ...
         validForkJoinMatch((GraphVo)this.graphMap.get(nodeId), forkSet, joinSet);
       }
     }
     tempPriorIdList = null;
   }
   
   private void validSimpleJoin(String graphId, Set<String> forkSet, Set<String> joinSet) {
     List<String> priorIdList = findPriorGraph(graphId);
     List<String> tempPriorIdList = new ArrayList();
     tempPriorIdList.addAll(priorIdList);
     for (String nodeId : tempPriorIdList) {
       ...
       if (!nodeId.startsWith("SJoinNode_")) {
         ...
         validDecisionSimpleJoinMatch((GraphVo)this.graphMap.get(nodeId), forkSet, joinSet);
       }
     }
   }
   ...
   private void validForkJoinMatch(GraphVo graph, Set<String> forkSet, Set<String> joinSet)
     throws IFinFlowException
   {
     List<String> priorNames = graph.getPriorNameList();
     int priorNamesSize = priorNames.size();
     ...
     for (int i = 0; i < priorNamesSize; i++)
     {
       ...
       if (priorName.startsWith("JoinNode_")) {
         validForkJoinMatch((GraphVo)this.graphMap.get(priorName), forkSet, joinSet);
       } else {
         ...
         if ((tempForkName != priorName) && (StringUtils.isNotBlank(tempForkName))) {
           throw new IFinFlowException(110014, new Object[0]);
         }
         if ((i == priorNamesSize - 1) && (tempForkName == priorName))
         {
           if (((GraphVo)this.graphMap.get(tempForkName)).getAfterNameList().size() != ((GraphVo)this.graphMap.get(currentJoinName)).getPriorNameList().size()) {
             throw new IFinFlowException(110014, new Object[0]);
           }
           if (!tempForkName.equals(((GraphVo)this.graphMap.get(currentJoinName)).getMatchId())) {
             throw new IFinFlowException(110017, new Object[] { currentJoinName });
           }
           deleteForkJoin(tempForkName, currentJoinName);
           forkSet.remove(tempForkName);
           joinSet.remove(currentJoinName);
           
           validForkJoinMatchs((GraphVo)this.graphMap.get(this.endId), forkSet, joinSet);
         }
       }
     }
   }
   ...
   public void getForkId(String nodeId, List<String> result, Set<String> forkOrDeciSet, Set<String> joinOrSimpSet, int flag)
   {
     ...
     if (!result.contains(nodeId))
     {
       if (nodeId.startsWith(joinType)) {
         validForkJoinMatch((GraphVo)this.graphMap.get(nodeId), forkOrDeciSet, joinOrSimpSet);
       }
     ...
     for (String pn : ((GraphVo)this.graphMap.get(nodeId)).getPriorNameList()) {
   ...
   {
     String afterName = (String)((GraphVo)this.graphMap.get(currentJoinName)).getAfterNameList().get(0);
     
 
     if (StringUtils.isNotBlank(afterName))
     {
       ((GraphVo)this.graphMap.get(currentJoinName)).setAfterNameList(null);
       
       ((GraphVo)this.graphMap.get(afterName)).getPriorNameList().remove(0);
       
 
       String priorNameBeforFork = (String)((GraphVo)this.graphMap.get(tempForkName)).getPriorNameList().get(0);
       
 
       ((GraphVo)this.graphMap.get(afterName)).getPriorNameList().add(priorNameBeforFork);
       
       ((GraphVo)this.graphMap.get(priorNameBeforFork)).getAfterNameList().remove(0);
       
       ((GraphVo)this.graphMap.get(priorNameBeforFork)).getAfterNameList().add(afterName);
   ...
   private void parseGraph(Element childElement) throws IFinFlowException {
     ...
     for (Element taskSubElement : XmlUtil.elements(childElement)) { String pthistory;
       if ("properties".equals(taskSubElement.getNodeName())) {
         for (Element taskSubEle : XmlUtil.elements(taskSubElement)) {
           if ("base".equals(taskSubEle.getNodeName())) {
             ...
             for (Element baseEle : XmlUtil.elements(taskSubEle)) {
               ...
               String subNodeValue = XmlHandle.dealString(baseEle.getTextContent());
               if (XmlHandle.isNotNull(subNodeValue)) {
                 if ("id".equals(subNodeName)) {
                   ...
                   intervalTime += Integer.parseInt(subNodeValue) * 24 * 60 * 60;
                 } else if ("intervalhour".equals(subNodeName)) {
                   intervalTime += Integer.parseInt(subNodeValue) * 60 * 60;
                 } else if ("intervalminute".equals(subNodeName)) {
                   intervalTime += Integer.parseInt(subNodeValue) * 60;
                 } else if ("intervalsecond".equals(subNodeName)) {
                   intervalTime += Integer.parseInt(subNodeValue);
                 } else if ("continueday".equals(subNodeName)) {
                   continueTime += Integer.parseInt(subNodeValue) * 24 * 60 * 60;
                 } else if ("continuehour".equals(subNodeName)) {
                   continueTime += Integer.parseInt(subNodeValue) * 60 * 60;
                 } else if ("continueminute".equals(subNodeName)) {
                   continueTime += Integer.parseInt(subNodeValue) * 60;
                 } else if ("continuesecond".equals(subNodeName)) {
                   continueTime += Integer.parseInt(subNodeValue);
                 }
               }
               else if ("adaptername".equals(subNodeName)) {
                 throw new IFinFlowException(110018, new Object[] { graph.getName(), graph.getId() });
               }
             }
             
             if (("timer".equals(childElement.getNodeName())) && (delayTime <= 0)) {
               throw new IFinFlowException(110019, new Object[] { graph.getName(), graph.getId() });
             }
             if ("2".equals(cycleType))
             {
               if ((Integer.parseInt(cycleCount) > 0) && 
                 (intervalTime <= 0)) {
                 throw new IFinFlowException(110020, new Object[] { graph.getName(), graph.getId() });
               }
             }
               ...
               (intervalTime != 0) && (continueTime != 0))
             {
               if (intervalTime > continueTime) {
                 throw new IFinFlowException(110021, new Object[] { graph.getName(), graph.getId() });
               }
             }
           }
           else if ("assigners".equals(taskSubEle.getNodeName()))
           {
             ...
             for (Element baseEle : XmlUtil.elements(taskSubEle)) {
               ...
               if ("pthistory".equals(baseEle.getNodeName())) {
                 ...
                 if ((1 != history) && (3 != history) && (4 != history) && (!XmlHandle.isNotNull(nodeValue)))
                 {
 
 
                   throw new IFinFlowException(110022, new Object[] { graph.getName(), graph.getId() });
                 }
         ...
         for (Attr transSubEleAttr : XmlUtil.attributes(taskSubElement)) {
           ...
           String subNodeValue = XmlHandle.dealString(transSubEleAttr.getValue());
     ...
     for (Map.Entry<String, GraphVo> entry : this.graphMap.entrySet()) {
       ...
       if (null != transitions) {
         ...
         for (int i = 0; i < size; i++) {
           ...
           ((GraphVo)this.graphMap.get(to)).getPriorNameList().add(key);
           ((GraphVo)this.graphMap.get(from)).getAfterNameList().add(to);
         }
       }
     }
   }
