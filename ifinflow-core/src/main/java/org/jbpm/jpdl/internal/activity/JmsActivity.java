...
public class JmsActivity extends JpdlAutomaticActivity {
	...
	public void perform(OpenExecution execution) {
		try {
			...
			Destination destination = (Destination) initialContext
					.lookup(this.destinationName);
			Object connectionFactory = initialContext
					.lookup(this.connectionFactoryName);

			if ((connectionFactory instanceof XAConnectionFactory)) {
				log.debug("connection factory '" + this.connectionFactoryName
						+ "' is a XAConnectionFactory: using xa jms apis");
				if ((destination instanceof Queue)) {
					log.debug("destination '" + this.destinationName
							+ "' is a Queue: using xa queue jms apis");
					XAQueueConnectionFactory xaQueueConnectionFactory = (XAQueueConnectionFactory) connectionFactory;
					sendToQueueXA((Queue) destination,
							xaQueueConnectionFactory, (ExecutionImpl) execution);
				} else if ((destination instanceof Topic)) {
					log.debug("destination '" + this.destinationName
							+ "' is a Topic: using xa topic jms apis");
					XATopicConnectionFactory xaTopicConnectionFactory = (XATopicConnectionFactory) connectionFactory;
					sendToTopicXA((Topic) destination,
							xaTopicConnectionFactory, (ExecutionImpl) execution);
				} else {
				...
				log.debug("connection factory '" + this.connectionFactoryName
						+ "' is a ConnectionFactory: using non-xa jms apis");
				if ((destination instanceof Queue)) {
					log.debug("destination '" + this.destinationName
							+ "' is a Queue: using non-xa queue jms apis");
					...
					log.debug("destination '" + this.destinationName
							+ "' is a Topic: using non-xa topic jms apis");
			...
			log.error("couldn't send jms message: " + e.getMessage(), e);
			throw new JbpmException("couldn't send jms message to queue"
					+ e.getMessage(), e);
		}
	}

	protected void sendToQueueXA(Queue queue,
		...
		try {
			xaQueueConnection = xaQueueConnectionFactory
	...
	protected void sendToTopicXA(Topic topic,
	...
	protected void sendToQueue(Queue queue,
		...
		try {
			...
			queueSession = queueConnection.createQueueSession(this.transacted,
	...
	protected void sendToTopic(Topic topic,
		...
		try {
			...
			topicSession = topicConnection.createTopicSession(this.transacted,
					this.acknowledgeMode);
	...
	protected Message createMessage(Session session, ExecutionImpl execution)
			throws Exception {
	...
	private MapMessage createMapMessage(Session session, ExecutionImpl execution)
			throws Exception {
		MapMessage result = session.createMapMessage();
		if (this.mapDescriptor != null) {
			List<Descriptor> keyDescriptors = this.mapDescriptor
					.getKeyDescriptors();
			List<Descriptor> valueDescriptors = this.mapDescriptor
					.getValueDescriptors();
			WireContext wireContext = new WireContext();
			wireContext.setScopeInstance(execution);
			for (int i = 0; i < keyDescriptors.size(); i++) {
				String key = (String) wireContext.create(
						(Descriptor) keyDescriptors.get(i), false);
				Object value = wireContext.create(
						(Descriptor) valueDescriptors.get(i), false);
	...
	private TextMessage createTextMessage(Session session,
			ExecutionImpl execution) throws Exception {
		Object value = this.textExpression.evaluate(execution);
		if (value != null) {
			return session.createTextMessage(value.toString());
		}
		throw new JbpmException("null value for expression '"
				+ this.textExpression + "' in jms activity");
	}

	private ObjectMessage createObjectMessage(Session session,
			ExecutionImpl execution) throws Exception {
		Object object = this.objectExpression.evaluate(execution);
		if ((object != null) && (!(object instanceof Serializable))) {

			throw new JbpmException(
					"can't send jms message: creation of object message expression '"
							+ this.objectExpression
							+ "' must be done with serializable: " + object);
		}
